...
module Monoids where
  ...
  equiv2 record { M = M ; ε = ε ; _⊗_ = _⊗_ } = record { M = M ; opers = ε , _⊗_ }
  ...
  equiv3 record { M = M ; opers = opers } = record { M = M ; opers = λ {(inj₁ x) → fst opers x
                                                                      ; (inj₂ y) → snd opers y} }
  ...
  equiv5 (falgebra carrier algebra) = record { M = carrier ; ε =  algebra (inj₁ tt) ; _⊗_ = λ x y → algebra (inj₂ (x , y)) }
  ...
  List-alg X = falgebra (List X) [ (λ x → []) , uncurry _++_ ]
...
module Maybe where
  ...
  Nat-alg3 : F-algebra
  Nat-alg3 = falgebra ℕ [ (λ _ → 1) , (λ x → x) ]

  open import Data.List hiding ([_])

  List-alg : (X : Set) → F-algebra
  List-alg X = falgebra ((List X)) [ (λ x → []) , (λ x → x) ]